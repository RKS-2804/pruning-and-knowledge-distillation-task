...
if __name__ == "__main__":
    ...
    parser.add_argument("--teacher_model_path", type=str, default=None, help="Path to the teacher model for distillation")
    parser.add_argument("--pruning_sparsity", type=float, default=0.0, help="Sparsity level for pruning (0.0 to 1.0)")
    parser.add_argument("--distillation_temperature", type=float, default=2.0, help="Temperature for distillation")
    parser.add_argument("--distillation_alpha", type=float, default=0.5, help="Alpha for combining distillation and task loss")
    args, left_argv = parser.parse_known_args()

    config = Config(args.config)
    config.argv_update(left_argv)

    # Parse dataset_name_or_paths if it's a string
    if hasattr(config, 'dataset_name_or_paths') and isinstance(config.dataset_name_or_paths, str):
        try:
            parsed = yaml.safe_load(config.dataset_name_or_paths)
            if isinstance(parsed, list):
                config.dataset_name_or_paths = parsed
            else:
                config.dataset_name_or_paths = [parsed]
    ...
    if args.distillation_temperature != 2.0:
        config.distillation_temperature = args.distillation_temperature
    if args.distillation_alpha != 0.5:
