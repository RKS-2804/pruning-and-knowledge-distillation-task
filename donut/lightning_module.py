...
class DonutModelPLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        if self.config.get("pruning_sparsity", 0.0) > 0:
            pruning_method = self.config.get("pruning_method", "magnitude")
            if pruning_method == "magnitude":
                self.model.apply_magnitude_pruning(self.config.pruning_sparsity)
                print(f"Applied {pruning_method} pruning with {self.config.pruning_sparsity} sparsity")
                self.pruning_masks = {}
                for name, param in self.model.get_non_embedding_parameters():
                    self.pruning_masks[name] = (param != 0).float()

        # Load teacher model for distillation if specified
        self.teacher_model = None
        if self.config.get("teacher_model_path", None):
            self.teacher_model = DonutModel.from_pretrained(
    ...
    def training_step(self, batch, batch_idx):
        ...
        student_outputs = self.model(image_tensors, decoder_input_ids, decoder_labels)
        loss = student_outputs.loss

        # Add distillation loss if teacher model is available
        if self.teacher_model is not None:
            self.teacher_model.to(image_tensors.device)  # Ensure teacher is on same device
            with torch.no_grad():
                teacher_outputs = self.teacher_model(image_tensors, decoder_input_ids, decoder_labels)

            # KL divergence loss between student and teacher logits
            distillation_temperature = self.config.get("distillation_temperature", 2.0)
            distillation_alpha = self.config.get("distillation_alpha", 0.5)

            student_logits = student_outputs.logits / distillation_temperature
            teacher_logits = teacher_outputs.logits / distillation_temperature
            ...
            # Combine losses
            loss = distillation_alpha * loss + (1 - distillation_alpha) * distillation_loss

            self.log_dict({"distillation_loss": distillation_loss, "combined_loss": loss}, sync_dist=True)

        self.log_dict({"train_loss": loss, "sparsity": self.model.calculate_sparsity()}, sync_dist=True)
    ...
    def configure_optimizers(self):
        ...
        # Register hook to reapply pruning masks after gradient updates
        if hasattr(self, 'pruning_masks') and self.pruning_masks:
            def reapply_masks(optimizer):
                for name, param in self.model.named_parameters():
                    if name in self.pruning_masks:
                        param.data.mul_(self.pruning_masks[name])
            optimizer.register_step_post_hook(reapply_masks)
