...
class AdapterBottleneck(nn.Module):
    """
    Adapter bottleneck layer to align dimensions between encoder and decoder.
    Reduces dimensionality to a bottleneck and then projects back.
    """
...
class DonutConfig(PretrainedConfig):
    ...
    Args:
        ...
        name_or_path:
            Name of a pretrained model name either registered in huggingface.co. or saved in local
        use_adapter:
            Whether to use the adapter bottleneck layer between encoder and decoder
        adapter_bottleneck_dim:
            Dimension of the bottleneck in the adapter layer
        distillation_temperature:
            Temperature for softening probabilities in distillation loss
        distillation_alpha:
            Weight for balancing original loss and distillation loss (0.0 to 1.0)
    """
...
class DonutModel(PreTrainedModel):
    ...
    def __init__(self, config: DonutConfig):
        ...
        encoder_dim = self.encoder.model.num_features
        decoder_dim = self.decoder.model.config.d_model

        # Add adapter bottleneck if enabled or dimensions mismatch
        if self.config.use_adapter or (encoder_dim != decoder_dim):
            self.adapter = AdapterBottleneck(encoder_dim, self.config.adapter_bottleneck_dim, decoder_dim)

    def forward(self, image_tensors: torch.Tensor, decoder_input_ids: torch.Tensor, decoder_labels: torch.Tensor, teacher_model: Optional["DonutModel"] = None):
        """
        ...
        if teacher_model is not None:
            with torch.no_grad():
                teacher_encoder_outputs = teacher_model.encoder(image_tensors)
                if hasattr(teacher_model, 'adapter'):
                    teacher_encoder_outputs = teacher_model.adapter(teacher_encoder_outputs)
                teacher_decoder_outputs = teacher_model.decoder(
                    input_ids=decoder_input_ids,
                    encoder_hidden_states=teacher_encoder_outputs,
                )
            teacher_logits = teacher_decoder_outputs.logits
            ...
            distillation_loss = F.kl_div(student_log_probs, teacher_probs, reduction='batchmean') * (self.config.distillation_temperature ** 2)
            if decoder_outputs.loss is not None:
                decoder_outputs.loss = self.config.distillation_alpha * decoder_outputs.loss + (1 - self.config.distillation_alpha) * distillation_loss
            else:
                decoder_outputs.loss = distillation_loss
    ...
    def get_non_embedding_parameters(self):
        """
        Get all parameters except embedding layers (embed_tokens, embed_positions, patch_embed)
        """
        non_embedding_params = []
        embedding_names = [
            'encoder.model.patch_embed.proj.weight',
            'encoder.model.patch_embed.proj.bias',
            'decoder.model.model.decoder.embed_tokens.weight',
            'decoder.model.model.decoder.embed_positions.weight',
            'decoder.model.lm_head.weight'
        ]
    ...
    def calculate_sparsity(self):
        """
        Calculate the current sparsity level (percentage of zero weights) for non-embedding parameters
        """
        total_params = 0
        zero_params = 0
    ...
    def apply_magnitude_pruning(self, sparsity_target: float):
        """
        Apply magnitude-based pruning to non-embedding parameters to achieve target sparsity

        Args:
            sparsity_target: Target sparsity level (0.0 to 1.0)
        """
        for name, param in self.get_non_embedding_parameters():
            if param.dim() > 1:  # Only prune matrices, not biases
    ...
    def apply_pruning_masks(self, masks: dict):
        """
        Apply pre-computed pruning masks to the model parameters

        Args:
            masks: Dictionary mapping parameter names to binary masks
        """
        non_emb_names = set(name for name, _ in self.get_non_embedding_parameters())
        for name, param in self.named_parameters():
            if name in masks and name in non_emb_names:
                param.data.mul_(masks[name])

    def save_pruning_masks(self, path: str):
        """
        Save the current pruning masks for non-embedding parameters to a file

        Args:
            path: Path to save the masks
        """
        masks = {}
        for name, param in self.get_non_embedding_parameters():
    ...
    def load_pruning_masks(self, path: str):
        """
        Load pruning masks from a file and apply them to the model

        Args:
            path: Path to load the masks from
        """
        masks = torch.load(path)
        self.apply_pruning_masks(masks)
