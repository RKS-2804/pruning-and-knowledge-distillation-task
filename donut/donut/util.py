...
class DonutDataset(Dataset):
    ...
    def __init__(
        ...
        # Initialize logging counters and timers
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.processed_count % 10 == 0:
            avg_parsing = self.total_parsing_time / self.processed_count
            avg_tokenization = self.total_tokenization_time / self.processed_count
            logging.info(f"Processed {self.processed_count} elements. Avg parsing time: {avg_parsing:.4f}s, Avg tokenization time: {avg_tokenization:.4f}s")
